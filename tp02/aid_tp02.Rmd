...
```{r}
d_distances %>% filter(mahalanobis > 30) %>% 
  ggplot(aes(sueldo, consumo_tc)) + geom_point() +
  scale_x_log10() + scale_y_log10()
```

```{r}
d_distances %>% 
  ggplot(aes(sueldo, consumo_tc, color = log(mahalanobis))) + geom_point() +
...
d_distances %>% ggplot(aes(sueldo, consumo_tc)) +
  geom_point() +
  geom_point(data = filter(d_distances, mahalanobis > 15), color = "red") +
  scale_y_log10() +
  scale_x_log10()
```
