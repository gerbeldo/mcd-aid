...
d %>%
  group_by(color) %>%
  # calculo la media para todas las columnas numericas. sin tener en cuenta NAs
  summarise(across(is.numeric, mean, na.rm = T)) %>%
  # convierto en tabla formato largo
  pivot_longer(-color, names_to = "var", values_to = "mean") %>%
  ggplot(aes(var, mean, color = color, group = color)) +
  geom_point() +
  geom_line()
```
...
d %>%
  ...
  # convierto en tabla formato largo
  pivot_longer(-color, names_to = "var", values_to = "median") %>%
  ggplot(aes(var, median, color = color, group = color)) +
  geom_point() +
  geom_line()
```
