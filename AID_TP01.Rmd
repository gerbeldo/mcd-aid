...
reca01 %>% 
  ...
  mutate(purchasetime = parse_date_time(purchasetime, "d b Y H M S")) -> reca01

reca02 %>% 
  janitor::clean_names() %>% 
  select(-x1) %>% 
  filter(purchaseamount > 0) %>% 
  mutate(purchasetime = parse_date_time(purchasetime, "d b Y H M S")) -> reca02

reca03 %>% 
  janitor::clean_names() %>% 
  select(-x1) %>% 
  filter(purchaseamount > 0) %>% 
  mutate(purchasetime = parse_date_time(purchasetime, "d b Y H M S")) -> reca03
```
